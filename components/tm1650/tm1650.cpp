...
float TM1650Display::get_setup_priority() const           { return setup_priority::DATA; }
...
uint8_t TM1650Display::print(uint8_t start_pos, const char *str) {
  ...
  for (; *str != '\0'; str++) {
    ...
    if (*str >= ' ' && *str <= '~') {
      char_data = TM1650_ASCII_TO_RAW[*str - ' '];
    }
...
void TM1650Display::clear() {
  for (uint8_t i = 0; i < this->length_; ++i) {
    if (this->backward_) {
      this->buffer_[(this->length_ - 1) - i] = 0;
    } else {
      this->buffer_[i] = 0;
    }
  }
...
void TM1650Display::set_dot(uint8_t pos, bool on) {
  uint8_t buf_pos = this->backward_ ? (this->length_ - 1) - pos : pos;
  if (pos >= this->length_) return;

  uint8_t dot_bit = this->segment_map_[7];

  if (on) {
    this->buffer_[buf_pos] |= dot_bit;
  } else {
    this->buffer_[buf_pos] &= ~dot_bit;
  }
}

void TM1650Display::print_time(uint8_t hour, uint8_t minute) {
  char buf[6];
